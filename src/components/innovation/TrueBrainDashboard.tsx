...
export default function TrueBrainDashboard() {
    ...
    <div className="w-full h-full flex bg-[#0B0F19] text-white font-sans overflow-hidden">
      ...
      <div className="w-[160px] md:w-[180px] shrink-0 border-r border-[#1f2937] flex flex-col p-3.5 bg-[#080c14] relative justify-between h-full">
        ...
        <div className="flex flex-col gap-1 mt-2 relative z-10">
          ...
          <div onClick={scrollToContact} className="px-3 py-2 rounded-lg flex items-center gap-3 text-white/40 hover:text-white/80 transition-colors cursor-pointer">
            <BarChart2 className="w-3.5 h-3.5" />
            <span className="text-[12px]">Insights</span>
          </div>
          <div onClick={scrollToContact} className="px-3 py-2 rounded-lg flex items-center gap-3 text-white/40 hover:text-white/80 transition-colors cursor-pointer">
            <Star className="w-3.5 h-3.5" />
            <span className="text-[12px]">Engagements</span>
          </div>
          <div onClick={scrollToContact} className="px-3 py-2 rounded-lg flex items-center gap-3 text-white/40 hover:text-white/80 transition-colors cursor-pointer">
            <FileText className="w-3.5 h-3.5" />
            <span className="text-[12px]">Invoices</span>
          </div>
          <div onClick={scrollToContact} className="px-3 py-2 rounded-lg flex items-center gap-3 text-white/40 hover:text-white/80 transition-colors cursor-pointer">
        ...
        <div className="flex-1 p-5 bg-gradient-to-b from-[#0B0F19] to-[#0a0d14] flex flex-col gap-4 overflow-hidden">
          ...
          <div className="bg-[#111827]/40 border border-[#1f2937]/80 rounded-xl p-4 flex flex-col flex-[1.3] min-h-0">
            <div className="flex justify-between items-center mb-1 shrink-0">
              <h3 className="text-[14px] font-serif text-white/90">Good morning.</h3>
            ...
            <div className="grid grid-cols-2 gap-6 flex-1 min-h-0">
              ...
              <div className="flex flex-col h-full min-h-0">
                ...
                <div className="flex-1 min-h-0 relative -ml-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={billedData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                      <defs>
              ...
              <div className="flex flex-col h-full min-h-0">
                <div className="mb-1 shrink-0">
                  <div className="text-[10px] text-white/50">Total Worked</div>
                  <div className="text-lg md:text-xl font-serif text-white tracking-wide mt-0.5">251.17 hrs</div>
                </div>
                <div className="flex-1 min-h-0 relative -ml-6">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={workedData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                      <defs>
          ...
          <div className="grid grid-cols-2 gap-4 flex-1 min-h-0">
            {/* Recent Workflows */}
            <div className="bg-[#111827]/40 border border-[#1f2937]/80 rounded-xl p-4 flex flex-col h-full min-h-0">
              <div className="flex justify-between items-center mb-2 shrink-0">
                <h3 className="text-[12px] font-serif text-white/90">Recent workflows</h3>
                <button className="text-[9px] text-blue-400 hover:text-blue-300 flex items-center gap-0.5 font-medium tracking-wide">
                  View all <ChevronRight className="w-2.5 h-2.5" />
                </button>
              </div>
              <div className="flex flex-col gap-2 flex-1 overflow-y-auto pr-1 no-scrollbar justify-center">
                <div className="bg-[#1f2937]/30 border border-white/5 rounded-lg p-2.5 flex items-center justify-between group hover:bg-[#1f2937]/50 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] text-white/70 group-hover:text-white/90 transition-colors truncate max-w-[130px] md:max-w-[180px]">Contract Review & Risk Analysis</span>
                  </div>
                  <span className="text-[8px] font-medium text-blue-400 bg-blue-900/20 px-2 py-0.5 rounded border border-blue-500/10 shrink-0">In Progress</span>
                </div>
                <div className="bg-[#1f2937]/30 border border-white/5 rounded-lg p-2.5 flex items-center justify-between group hover:bg-[#1f2937]/50 transition-colors cursor-pointer">
                  <div className="flex items-center gap-3">
                    <div className="p-1.5 bg-white/5 rounded">
            ...
            <div className="bg-[#111827]/40 border border-[#1f2937]/80 rounded-xl p-4 flex flex-col h-full min-h-0">
              <div className="flex justify-between items-center mb-2 shrink-0">
                <h3 className="text-[12px] font-serif text-white/90">New Team Members</h3>
                <button className="text-[9px] text-blue-400 hover:text-blue-300 flex items-center gap-0.5 font-medium tracking-wide">
                  View all <ChevronRight className="w-2.5 h-2.5" />
                </button>
              </div>
              <div className="flex flex-col gap-2.5 flex-1 overflow-y-auto pr-1 no-scrollbar justify-center">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-full bg-[#1f2937]/50 border border-white/5 flex items-center justify-center">
                        <UserCircle2 className="w-3.5 h-3.5 text-white/30" />
                      </div>
                      <div className="flex flex-col gap-1">
                        <div className="w-14 h-1.5 rounded bg-white/10"></div>
                        <div className="w-8 h-1 rounded bg-white/5"></div>
                      </div>
                    </div>
                    <span className="text-[8px] font-medium text-purple-400 bg-purple-900/20 px-2 py-0.5 rounded border border-purple-500/10 shrink-0">Onboarding</span>
                  </div>
                ))}
              </div>
            </div>
