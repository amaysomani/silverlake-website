...
export default function HomePageContent({ practiceAreas, articles }: Props) {
  return (
    <>
      <Header />
              ...
              <div className="mx-auto max-w-[1400px] px-6 lg:px-10">
                <div className="flex flex-col sm:flex-row justify-between items-baseline mb-16 gap-6">
                  ...
                  <MagneticButton strength={0.2}>
                    <Link href="/practice-areas" className="group relative inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-[#C5A059] transition-colors py-2">
                  ...
                  {practiceAreas.map((area, idx) => (
                    <motion.div key={area.slug} variants={fadeInUp} whileHover={{ y: -12 }} className="group relative border border-white/10 aspect-[3/4] overflow-hidden bg-black shadow-lg hover:shadow-[0_20px_40px_rgba(0,0,0,0.5)] transition-all duration-700">
                      <Link href={`/practice-areas/${area.slug}`} className="block h-full w-full">
                        <img src={`https://images.unsplash.com/photo-${["1589829085413-56de8ae18c73", "1454165804606-c3d57bc86b40", "1497366216548-37526070297c", "1600880292203-757bb62b4baf"][idx % 4]}?q=80&w=800&auto=format&fit=crop`} alt={area.name} className="absolute inset-0 w-full h-full object-cover opacity-30 group-hover:scale-[1.08] group-hover:opacity-50 transition-all duration-[1.2s] ease-[cubic-bezier(0.16,1,0.3,1)]" />
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-8 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-700 ease-[cubic-bezier(0.16,1,0.3,1)]">
                          <h3 className="font-serif text-2xl font-light mb-4 text-white group-hover:text-[#C5A059] transition-colors duration-500 relative z-10">{area.name}</h3>
                          <div className="h-[2px] w-0 bg-[#C5A059] group-hover:w-12 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] mb-4" />
                          <p className="text-sm text-white/50 leading-relaxed group-hover:text-white/80 transition-colors duration-500 relative z-10">{area.overview.substring(0, 80)}...</p>
                        </div>
                      </Link>
                    </motion.div>
                  ))}
          ...
          <div className="sticky top-0 min-h-screen w-full bg-[#fcfbf9] z-40 shadow-[0_-20px_50px_rgba(0,0,0,0.4)] py-[100px] lg:py-[140px] flex flex-col justify-center">
            <div className="mx-auto max-w-[1400px] px-6 lg:px-10 w-full">
              <div className="flex flex-col sm:flex-row justify-between items-baseline mb-16 gap-6 border-b border-black/10 pb-6">
                ...
                <MagneticButton strength={0.2}>
                  <Link href="/insights" className="group relative inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.15em] text-[#0A1128] transition-colors py-2">
              ...
              <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={staggerContainer} className="grid grid-cols-1 md:grid-cols-3 gap-12">
                {articles.map((art) => (
                  <motion.article key={art.slug} variants={fadeInUp} whileHover={{ y: -12 }} className="group cursor-pointer">
                    <Link href={`/insights/${art.slug}`} className="block h-full border-t border-transparent transition-colors">
                      <div className="aspect-[4/3] overflow-hidden mb-6 relative shadow-md group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.15)] transition-all duration-700">
                        <img src={art.featuredImage} alt={art.title} className="w-full h-full object-cover group-hover:scale-[1.08] transition-transform duration-[1.2s] ease-[cubic-bezier(0.16,1,0.3,1)]" />
                      </div>
                      <div className="flex items-center gap-4 mb-4">
                        <span className="text-[10px] uppercase tracking-[0.2em] text-[#C5A059] font-semibold">{art.category}</span>
                        <div className="w-1 h-1 rounded-full bg-black/20" />
                        <span className="text-[10px] uppercase tracking-[0.1em] text-[#0A1128]/50">{new Date(art.datePublished).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                      </div>
                      <h3 className="font-serif text-2xl font-light text-[#0A1128] mb-4 group-hover:text-[#C5A059] transition-colors duration-500 leading-snug">{art.title}</h3>
            ...
            <section className="py-[100px] lg:py-[140px] bg-[#1F2A44] text-[#fcfbf9] text-center overflow-hidden">
              <div className="mx-auto max-w-[1000px] px-6 lg:px-10 relative z-10">
                ...
                <MagneticButton strength={0.3}>
                  <Link href="/careers" className="inline-flex items-center gap-3 bg-[#C5A059] text-[#0A1128] px-8 py-5 text-xs font-semibold uppercase tracking-[0.2em] hover:bg-white transition-colors duration-500 shadow-xl">
